...
class LaunchpadUI {
  ...
  openPurchase(launch) {
    if (deploymentConfig.mode !== 'testnet') {
      this.notify('Wallet actions are unavailable until a reviewed testnet manifest is loaded.');
      return;
    }
  ...
  render() {
    ...
    if (list.length === 0) grid.append(Object.assign(document.createElement('p'), { className: 'empty-state', textContent: 'No indexed testnet launches are currently available.' }));
  ...
  launchCard(launch) {
    ...
    const stats = document.createElement('div'); stats.className = 'launch-stats'; stats.innerHTML = `<span><b>${launch.raised.toFixed(1)} TON</b> raised</span><span><b>${launch.participants}</b> participants</span><span><b>${formatSaleUnits(launch.remainingSaleUnits)}</b> units left</span>`;
    const foot = document.createElement('div'); foot.className = 'launch-foot'; const label = document.createElement('span'); label.textContent = 'Fixed-price testnet sale'; const action = document.createElement('button'); action.className = 'button button-card'; action.type = 'button'; action.textContent = 'Review purchase'; action.addEventListener('click', () => this.openPurchase(launch)); foot.append(label, action);
    card.append(top, stats, foot); return card;
  }
}
...
function formatSaleUnits(value) {
  try { return BigInt(value).toLocaleString('en-US'); } catch { return '—'; }
}
